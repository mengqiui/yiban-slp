<template>
  <div class="login-box">
    ...
    <form @submit.prevent="onsubmit" class="needs-validation">
      <p class="h4 text-center mb-5">注册新用户</p>
  ...
  methods: {
    ...
    onsubmit() {
      ...
      this.$validator.validateAll().then((result) => {
        if (result) {
          this.$axios.post('userRegister.do',this.$qs.stringify({ userName: this.form.username,password:this.$md5('(<>::dkj2154|'+this.form.passwd2+'.4543,%^%$@;d458,l'),mobileCode:this.form.mobileCode}))
          .then(function (response) {
            if (response.status === 200) {
              var data = that.$qs.parse(response.data)
